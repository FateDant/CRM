...
Grid.prototype.build = function() {
	...
	this.renderTo.html("");
	// 生成TABLE元素
	var table = $("<table class='grid' cellspacing='0' cellpadding='0'></table>").appendTo(this.renderTo);
	...
	var arrHTML = [];
	arrHTML.push("<tr><td class='info-show'>共 <span class='info-total'>" + this.dataSource.total + "</span> 项,每页显示</td>");
	arrHTML.push("<td class='ddlPageSize-container'><div class='ddlPageSize'></div></td><td class='pageItem'>项</td>");
	arrHTML.push("<td class='btnContainer'><input type='button' class='btnPrev pagerBtn' value='上一页' /></td>");
	arrHTML.push("<td class='currentPage'><span class='lblCurrentPage'>" + this.postData.pageNum + "</span>");// 当前第几页
	arrHTML.push("/<span class='lblTotalPage'>" + Math.ceil(this.dataSource.total / this.postData.pageSize) + "</span></td>");// 总共多少页
	arrHTML.push("<td class='nextPage'><input type='button' class='btnNext pagerBtn' value='下一页' /></td></tr>");
...
Grid.prototype.eventBind = function() {
	...
	$(".btnNext", grid).click(function() {
		...
		// 新页码是当前页码+1
		var newPage = Math.min(maxPage, parseInt(page) + 1);
		// 将新页码填进元素
		cp.text(newPage);
		// 重新加载表格
